...
var SettingsProcess = /** @class */ (function (_super) {
    ...
    SettingsProcess.prototype.swapSettingsTab = function (targetModuleID) {
        var targetModuleSettings = undefined;
        for (var _i = 0, _a = Array.from(this.moduleSettingsList.values()); _i < _a.length; _i++) {
            var moduleSettings = _a[_i];
        ...
        if (targetModuleSettings === undefined) {
            throw new Error("Could not locate module settings for ".concat(targetModuleID));
        }
        var settingsList = targetModuleSettings.getSettingsAndHeaders();
        var list = {
                ...
                switch (_c.label) {
                    case 0:
                        ...
                        switch (_a) {
                            ...
                            case "on-check-for-update-press": return [3 /*break*/, 5];
                    ...
                    case 6:
                        response = _c.sent();
                        if (![nexus_module_builder_1.HTTPStatusCodes.OK, nexus_module_builder_1.HTTPStatusCodes.NO_CONTENT].includes(response.code)) {
                            console.error("[Nexus Settings] An error occurred when checking for an update for ".concat(moduleID));
                            console.error("[Nexus Settings]\t".concat(response.body));
                            return [2 /*return*/, false];
                        }
                        if (response.code === nexus_module_builder_1.HTTPStatusCodes.OK) { // update found
                            console.info("[Nexus Settings]\t".concat(response.body));
                            return [2 /*return*/, true];
                        }
                        if (response.code === nexus_module_builder_1.HTTPStatusCodes.NO_CONTENT) { // current version is higher or at the latest version
                            console.info("[Nexus Settings]\t".concat(response.body));
                            return [2 /*return*/, false];
                        }
                        _c.label = 7;
